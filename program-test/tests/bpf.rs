use {
    ...
    solana_sdk::{
        bpf_loader, bpf_loader_upgradeable, instruction::Instruction, pubkey::Pubkey,
        signature::Signer, transaction::Transaction,
    },
};

#[tokio::test]
async fn test_bpf_program_loader_v2() {
...
#[tokio::test]
async fn test_bpf_program_loader_v3() {
    let program_id = Pubkey::new_unique();
    ...
    let mut program_test = ProgramTest::default();
    program_test.prefer_bpf(true);
    program_test.prefer_loader_v3(true); // Loader v3
    program_test.add_program("noop_aligned", program_id, None);

    let mut context = program_test.start_with_context().await;

    // Assert the program is a BPF Loader 3 program.
    let program_account = context
        .banks_client
        .get_account(program_id)
        .await
