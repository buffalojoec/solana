use {
    clap::Parser, prost::Message, solana_svm_fuzz_harness_fixture::proto::ElfLoaderFixture,
    std::path::PathBuf,
};
...
fn exec(input: &PathBuf) -> bool {
    ...
    let Some(effects) = solana_svm_fuzz_harness_vm::elf_loader::execute_elf_loader_proto(context)
    else {
        println!(
