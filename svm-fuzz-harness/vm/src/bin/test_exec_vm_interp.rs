use {
    clap::Parser, prost::Message, solana_svm_fuzz_harness_fixture::proto::SyscallFixture,
    std::path::PathBuf,
};
...
fn exec(input: &PathBuf) -> bool {
    ...
    let Some(effects) = solana_svm_fuzz_harness_vm::interp::execute_vm_interp_proto(context) else {
        println!(
