//! Main harness for executing Agave's VM intepreter entrypoint using a
//! Protobuf instruction context.

use {
    crate::utils::vm::{err_map, mem_regions, HEAP_MAX, STACK_SIZE},
    bincode::Error,
    prost::Message,
    ...
    solana_svm_fuzz_harness_fixture::{
        ...
        proto::{
            SyscallContext as ProtoSyscallContext, SyscallEffects as ProtoSyscallEffects,
            VmContext as ProtoVmContext,
        },
    },
    std::{borrow::Borrow, cell::RefCell, ffi::c_int, sync::Arc},
};
...
fn setup_internal_fn_registry(
    ...
    for (byte_idx, byte) in call_whitelist.iter().enumerate() {
        for bit_idx in 0..8 {
            if (byte & (1 << bit_idx)) != 0 {
                let pc = byte_idx.checked_mul(8).unwrap().saturating_add(bit_idx);
...
pub fn execute_vm_interp_proto(input: ProtoSyscallContext) -> Option<ProtoSyscallEffects> {
    let mut instr_ctx: InstrContext = input.instr_ctx?.try_into().ok()?;
    let mut feature_set = instr_ctx.epoch_context.feature_set;
    ...
    if executable.verify::<RequisiteVerifier>().is_err() {
        return Some(ProtoSyscallEffects {
            error: -2,
            ..Default::default()
        });
    }
    ...
    let mut context_obj = TestContextObject::new(instr_ctx.compute_units_available);
...
pub unsafe extern "C" fn sol_compat_vm_interp_v1(
    ...
    let Some(syscall_effects) = execute_vm_interp_proto(syscall_context) else {
        return 0;
    };
