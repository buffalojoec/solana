//! VM memory region management.

use {
    solana_program_runtime::solana_rbpf::{
        aligned_memory::AlignedMemory,
        ebpf::{self, HOST_ALIGN},
        memory_region::{MemoryMapping, MemoryRegion, MemoryState},
    },
    solana_svm_fuzz_harness_fixture::proto::InputDataRegion as ProtoInputDataRegion,
};
...
pub fn setup_input_regions(
    ...
    input_data_regions: &Vec<ProtoInputDataRegion>,
) {
    ...
    for input_data_region in input_data_regions {
        ...
        if input_data_region.is_writable {
            regions.push(MemoryRegion::new_writable(
                mem.as_slice_mut(),
                ebpf::MM_INPUT_START.saturating_add(input_data_off),
            ));
        } else {
        ...
        input_data_off = input_data_off.saturating_add(input_data_region.content.len() as u64);
    }
}
...
pub fn extract_input_data_regions<'a>(mapping: &'a MemoryMapping<'a>) -> Vec<ProtoInputDataRegion> {
    match mapping {
        ...
        MemoryMapping::Unaligned(mapping) => {
            ...
            let mut input_regions: Vec<ProtoInputDataRegion> = mapping
                .get_regions()
                .iter()
