...
use {
    solana_bpf_loader_program::syscalls::SyscallError,
    ...
    solana_svm_fuzz_harness_fixture::proto::ErrKind as ProtoErrKind,
};
...
pub fn unpack_stable_result(
    ...
    invoke_context: &InvokeContext,
    program_id: &Pubkey,
) -> (i64, ProtoErrKind, u64) {
    match program_result {
        StableResult::Ok(n) => (0, ProtoErrKind::Unspecified, n),
        StableResult::Err(ref err) => {
            ...
            let (err_no, err_kind) = if let EbpfError::SyscallError(syscall_error) = err {
                if let Some(instruction_err) = syscall_error.downcast_ref::<InstructionError>() {
                    ...
                    (instr_err_to_num(instruction_err), ProtoErrKind::Instruction)
                    ...
                    (syscall_err_to_num(syscall_error), ProtoErrKind::Syscall)
                    ...
                    .is_some()
                {
                    ...
                    (-1, ProtoErrKind::Unspecified)
                }
            } else {
                stable_log::program_failure(&logger, program_id, &ebpf_err_to_str(err));
                (ebpf_err_to_num(err), ProtoErrKind::Ebpf)
            };
