    ...
    quote! {
        ...
        mod #test_mod_ident {
            ...
            fn test_abi_digest() {
                ...
                // pretty-print error
                if result.is_err() {
                    ::solana_frozen_abi::log::error!("digest error: {:#?}", result);
                }
                result.unwrap();
                ...
                if ::std::env::var("SOLANA_ABI_BULK_UPDATE").is_ok() {
                    ...
                    ::solana_frozen_abi::log::warn!("Not testing the abi digest under SOLANA_ABI_BULK_UPDATE!");
                } else {
