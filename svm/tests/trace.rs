...
fn test_processed_transactions() {
    ...
    struct TestHandler {
        ...
        seen_signatures_from_traces: RwLock<HashSet<Signature>>,
    }
    impl TraceHandler for TestHandler {
        ...
        fn digest_trace(&self, trace: &STFTrace<impl SVMTransaction>) {
            ...
            if let STFTrace::Directive(directive) = trace {
                self.seen_signatures_from_traces
                    .write()
                    .unwrap()
...
fn test_proofs() {
    ...
    let traces_trie = rollup.trace_handler().traces_trie.read().unwrap();
    let traces_merkle_tree = traces_trie.merklize();
    ...
    for (i, res) in result.processing_results.iter().enumerate() {
        ...
        let (index, candidate) = {
            // First hash the trace entry manually.
            ...
            let trace_hash = hasher.result_reset();

            // Get the leaf index for this hash.
            let index = traces_trie.get_leaf_index(&trace_hash).unwrap();

            // Hash with the leaf prefix.
            hasher.hashv(&[&[0], trace_hash.as_ref()]);
            let trace_hash_with_prefix = hasher.result_reset();

            (index, trace_hash_with_prefix)
        };
        let proof = traces_merkle_tree.find_path(index).unwrap();
