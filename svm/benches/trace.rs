...
#[derive(Default)]
struct TestSignatureHandler {
    signatures_trie: RwLock<Trie>,
}
impl TraceHandler for TestSignatureHandler {
...
#[derive(Default)]
struct TestReceiptHandler {
    receipts_trie: RwLock<Trie>,
}
impl TraceHandler for TestReceiptHandler {
    fn digest_transaction(&self, _transaction: &impl SVMTransaction) {
        // No-op.
    }
...
fn trace(c: &mut Criterion) {
    ...
    let rollup_with_signature_handler = MockRollup::<TestSignatureHandler>::default();
    let rollup_with_receipt_handler = MockRollup::<TestReceiptHandler>::default();
    let rollup_with_trace_handler = MockRollup::<TestTraceHandler>::default();
    ...
    for (set_name, transaction_set) in transaction_sets {
        ...
        group.bench_function(
            format!("{} Transaction Batch: With Signature Hashing", set_name),
            |b| {
                b.iter(|| {
                    batch_processor.load_and_execute_sanitized_transactions(
                        &rollup_with_signature_handler, // Signature hashing handlers.
                        santitized_txs,
                        check_results.clone(),
                        &processing_environment,
                        &processing_config,
        ...
        group.bench_function(
            ...
            |b| {
                b.iter(|| {
                    batch_processor.load_and_execute_sanitized_transactions(
                        &rollup_with_receipt_handler, // Receipt hashing handlers.
                        santitized_txs,
                        check_results.clone(),
                        &processing_environment,
                        &processing_config,
        ...
        // With trace hashing.
        group.bench_function(
            format!("{} Transaction Batch: With Trace Hashing", set_name),
            |b| {
                b.iter(|| {
                    batch_processor.load_and_execute_sanitized_transactions(
                        &rollup_with_trace_handler, // Trace hashing handlers.
                        santitized_txs,
                        check_results.clone(),
                        &processing_environment,
                        &processing_config,
