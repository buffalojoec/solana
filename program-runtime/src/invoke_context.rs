...
impl<'a, 'ix_data> InvokeContext<'a, 'ix_data> {
    ...
    /// Process a message. Calls each instruction in the message over the
    /// configured [`TransactionContext`] and returns the final result.
    /// For any instructions that fail, a tuple is returned whose elements are
    /// (index of instruction, instruction error). Once an error is returned,
    /// execution stops.
    ...
    fn process_executable_chain(
        ...
        let mut vm = EbpfVm::new(
            Arc::clone(&**self.environment_config.program_runtime_environment),
            SBPFVersion::V0,
    ...
    /// Get this invocation's program runtime environment.
    pub fn get_program_runtime_environment(&self) -> &ProgramRuntimeEnvironment {
        self.environment_config.program_runtime_environment
    }
...
macro_rules! with_mock_invoke_context_with_feature_set {
    (
        ...
        let program_runtime_environment =
            $crate::loaded_programs::ProgramRuntimeEnvironment::mock();
...
mod tests {
    ...
    fn test_process_message_readonly_handling() {
        ...
        let program_runtime_environment = ProgramRuntimeEnvironment::mock();
        let environment_config = EnvironmentConfig::new(
            Hash::default(),
            0,
            false,
        ...
        let result =
            invoke_context.process_message(&message, &mut ExecuteTimings::default(), &mut 0);
        assert!(result.is_ok());
        assert_eq!(
            transaction_context
                .accounts()
