...
impl BuiltinPrograms {
    pub fn new(builtins: Vec<BuiltinPrototype>) -> BuiltinPrograms {
        BuiltinPrograms(builtins)
    }

    pub fn iter(&self) -> impl Iterator<Item = &BuiltinPrototype> {
        self.0.iter()
    }

    pub fn get_program_ids(&self) -> HashSet<Pubkey> {
        self.0.iter().map(|p| p.program_id).collect()
    }
}
