...
fn check_upgradeable_loader_state(
    ...
    Err(MigrateNativeProgramError::InvalidProgramDataAccount(
...
mod tests {
    ...
    fn test_bpf_upgradeable_program_config() {
        ...
        assert_matches!(
            BpfUpgradeableProgramConfig::new_checked(&bank, &program_id).unwrap_err(),
            MigrateNativeProgramError::FailedToDeserialize(_)
        );
        ...
        assert_matches!(
            BpfUpgradeableProgramConfig::new_checked(&bank, &program_id).unwrap_err(),
            MigrateNativeProgramError::ProgramHasNoDataAccount(_)
        );
        ...
        assert_matches!(
            BpfUpgradeableProgramConfig::new_checked(&bank, &program_id).unwrap_err(),
            MigrateNativeProgramError::AccountIsExecutable(_)
        );
