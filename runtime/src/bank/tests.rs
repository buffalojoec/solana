...
fn test_override_builtins() {
    let check_bank_builtins = |builtins: &[BuiltinPrototype]| {
        let bank = Bank::new_for_tests_with_config(
            &GenesisConfig::default(),
            BankTestConfig {
                override_builtins: Some(builtins),
                ..BankTestConfig::default()
            }
        );
        assert_eq!(
            bank.builtin_programs.len(),
            builtins.len(),
        );
        builtins
            .iter()
    ...
    let check_bank_builtin_feature_activation = |builtins: &[BuiltinPrototype]| {
        ...
        feature_ids
            ...
            .for_each(|feature_id| {
                ...
                bank.store_account(feature_id, &feature::create_account(&Feature::default(), 42));
            });
        bank.feature_set = Arc::new(feature_set.clone());

        // Assert the builtins have not been enabled yet.
        assert!(bank.builtin_programs.is_empty());

        bank.apply_feature_activations(ApplyFeatureActivationsCaller::NewFromParent, false, Some(builtins));
